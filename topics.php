							...
							?>
								<li class="list-group-item mb-4">
									<div>
										...
										<?php endif; ?>
										<span class="float-right mr-4"><small><i>Date: <?php echo date('M d, Y h:i A', strtotime($row['date_created'])) ?></i></small></span>
									...
									<span class="float-left badge badge-secondary text-white"><?php echo number_format($view) ?> Views</span>
									<span class="float-left badge badge-primary text-white ml-2"><i class="fa fa-comments"></i> <?php echo number_format($comments) ?> Comments <?php echo $replies > 0 ? " and " . number_format($replies) . ' Replies' : '' ?> </span>
...
<script>
	...
	$('#new_topic').click(function() {
		uni_modal("New Topic", "manage_topic.php", 'mid-large')
	})
	...
	function delete_topic($id) {
		...
		$.ajax({
			...
			success: function(resp) {
				if (resp == 1) {
					alert_toast("Operation successful", 'success')
					setTimeout(function() {
						location.reload()
					}, 1500)
